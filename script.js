...
async function registrarBaixa() {
    ...
    await addDoc(collection(db,"baixasMedicamentos"), {
        militar,
        medicamento,
        quantidade,
        observacao,
...
async function carregarBaixas(){
    ...
    const snap = await getDocs(collection(db,"baixasMedicamentos"));
...
async function excluirBaixa(id){
    ...
    await deleteDoc(doc(db,"baixasMedicamentos",id));
...
function gerarRelatorio(){
    ...
    pdf.setFontSize(16);
    pdf.text("Relatorio de Medicamentos", 15, 15);
    ...
    baixas.forEach(item=>{
        ...
        let nome = item.medicamento.toUpperCase();
    ...
    Object.keys(totais).forEach(med=>{
        ...
        pdf.text(`${med} ........ Quantidade: ${totais[med]}`, 15, y);

        y += 10;

    });

    pdf.save("relatorio_medicamentos.pdf");
